...
export function buildAssignedGoldEconomicCreditSummary(events=[]){
  ...
  for(const e of resolved){
    for(const r of e.recipients??[]){
      ...
      const out=byPlayer[key]??={pawnEntityIndex:r.pawnEntityIndex,playerName:r.playerName??null,steamId:r.steamId??null,heroId:r.heroId??null,team:r.team??null,creditEvents:0,observedRecipientTransitions:0,observedCurrency0DeltaTotal:0};
      out.creditEvents++;out.observedRecipientTransitions++;out.observedCurrency0DeltaTotal+=finite(r.delta)??0;
    }
  }
