...
test('production capability contract owns every A metric exactly once',()=>{
  ...
  assert.equal(PRODUCTION_CAPABILITIES.filter(c=>c.productionStatus==='supported').flatMap(c=>c.metricIds).length,55);
});

test('pipeline requires fresh outputs and records 55/68 production coverage',async()=>{
  ...
  assert.equal(result.productionManifest.coverage.completeAuthoritative,55);
  assert.equal(result.productionManifest.coverage.notSupportedAuthoritative,13);
