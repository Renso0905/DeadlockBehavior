...
export async function buildReplayModel({outputRoot,replayName,cacheRoot=null,force=false}) {
  ...
  const runtimeFlyingSouls=await readJson(productionPath('runtime_flying_soul_production_v01.json'));
  ...
  const model={
    ...
    troopers,
    flyingSouls:runtimeFlyingSouls?.status==='RUNTIME_FLYING_SOUL_PRODUCTION_V01_READY'?{status:runtimeFlyingSouls.status,authorityLayer:'extended',summary:runtimeFlyingSouls.summary,counts:runtimeFlyingSouls.counts,damageDiagnostics:runtimeFlyingSouls.damageDiagnostics,semanticScope:runtimeFlyingSouls.semanticScope,validation:runtimeFlyingSouls.validation,source:'runtime_flying_soul_production_v01.json'}:null,
