...
export function attachMetricAvailability(model){
  ...
  for(const metric of PRODUCTION_METRIC_CONTRACT.metrics){
    ...
    for(const player of model.players){
      ...
      if(!why&&['orb_secure','orb_deny'].includes(metric.metricId)&&!model.flyingSouls?.summary?.damageObservation?.singleTeamOutcome)why='Validated flying-soul single-team outcomes were not produced.';
