...
export function associatePlayerDamage(events,damageMessages,playerByPawn,{matchClockOffsetSeconds=0,tickRate=64}={}){
  ...
  for(const message of damageMessages??[]){
    ...
    candidates[0].playerDamageMessages.push({sequence:Number.isInteger(message.sequence)?message.sequence:null,tick:Number(message.tick),matchTimeSeconds:Number(message.tick)/tickRate-matchClockOffsetSeconds,messageType:message.messageType,victimIndex:Number(message.victimIndex),attackerIndex:Number(message.attackerIndex),attackerPlayer:{playerId:player.playerId,controllerEntityIndex:Number(player.controllerEntityIndex),pawnEntityIndex:Number(player.pawnEntityIndex),playerName:player.playerName,team:Number(player.team),heroId:player.heroId==null?null:Number(player.heroId)}});
  }
  const byPlayer=new Map();
  for(const event of events??[]){
    event.playerDamageMessages.sort((a,b)=>a.tick-b.tick||(a.sequence??0)-(b.sequence??0));
    const players=new Set(),teams=new Set();
    for(const message of event.playerDamageMessages){const p=message.attackerPlayer;players.add(p.playerId);teams.add(p.team);if(!byPlayer.has(p.playerId))byPlayer.set(p.playerId,{...p,damagedEpisodes:new Set(),damageMessages:0,secureEpisodes:0,denyEpisodes:0});const row=byPlayer.get(p.playerId);row.damagedEpisodes.add(event.episodeId);row.damageMessages++;}
    ...
    const first=event.playerDamageMessages[0]??null;
    if(teams.size===1&&first&&[2,3].includes(Number(event.team))){
      const outcome=Number(first.attackerPlayer.team)===Number(event.team)?'DENY':'SECURE';
      event.singleTeamOutcome={status:'RESOLVED_SINGLE_OBSERVED_TEAM_FIRST_HIT',outcome,confidence:'HIGH',firstHit:structuredClone(first),observedAttackerTeam:Number(first.attackerPlayer.team),orbTeam:Number(event.team)};
      const credited=byPlayer.get(first.attackerPlayer.playerId);if(credited)credited[outcome==='SECURE'?'secureEpisodes':'denyEpisodes']++;
    }else event.singleTeamOutcome={status:teams.size>1?'MIXED_TEAM_PROVISIONAL':first?'INVALID_ORB_TEAM_UNRESOLVED':'NO_PLAYER_DAMAGE_OBSERVED',outcome:null,confidence:null,firstHit:first?structuredClone(first):null,observedAttackerTeam:teams.size===1?Number(first.attackerPlayer.team):null,orbTeam:Number.isFinite(Number(event.team))?Number(event.team):null};
  }
  const episodes=events?.length??0,withDamage=(events??[]).filter(x=>x.damageObservation.observedPlayerDamage).length;
  const summary={episodesWithObservedPlayerDamage:withDamage,episodesWithoutObservedPlayerDamage:episodes-withDamage,totalObservedPlayerDamageMessages:(events??[]).reduce((n,x)=>n+x.damageObservation.damageMessageCount,0),multiMessageEpisodes:(events??[]).filter(x=>x.damageObservation.multiMessage).length,multiPlayerEpisodes:(events??[]).filter(x=>x.damageObservation.multiPlayer).length,mixedTeamEpisodes:(events??[]).filter(x=>x.damageObservation.mixedTeam).length,singleTeamOutcome:{resolvedEpisodes:(events??[]).filter(x=>x.singleTeamOutcome.outcome!==null).length,secureEpisodes:(events??[]).filter(x=>x.singleTeamOutcome.outcome==='SECURE').length,denyEpisodes:(events??[]).filter(x=>x.singleTeamOutcome.outcome==='DENY').length,mixedTeamProvisionalEpisodes:(events??[]).filter(x=>x.singleTeamOutcome.status==='MIXED_TEAM_PROVISIONAL').length,noPlayerDamageObservedEpisodes:(events??[]).filter(x=>x.singleTeamOutcome.status==='NO_PLAYER_DAMAGE_OBSERVED').length},byPlayer:[...byPlayer.values()].map(x=>({...x,damagedEpisodes:x.damagedEpisodes.size})).sort((a,b)=>a.controllerEntityIndex-b.controllerEntityIndex)};
