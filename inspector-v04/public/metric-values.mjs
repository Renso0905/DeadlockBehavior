...
function rawMetricValue(id,{model,p,time,weaponReady}){
  ...
  switch(id){
    ...
    case 'trooper_orbs': return v(model.flyingSouls?.summary?.episodes??'—','Replay-wide mutually unique Trooper-death → CItemXP subclass 494398941 links');
    case 'orb_attackable_window': { const fs=model.flyingSouls?.summary??{}; return v(fs.medianAttackableWindowSeconds!=null?duration(fs.medianAttackableWindowSeconds):'—',`${fs.attackableWindows??0}/${fs.episodes??0} direct attackable windows · min ${duration(fs.minAttackableWindowSeconds)} · max ${duration(fs.maxAttackableWindowSeconds)}`); }
    case 'orb_shot_no_shot': { const d=model.flyingSouls?.summary?.damageObservation??{}; return v(`${d.episodesWithObservedPlayerDamage??0} observed damage · ${d.episodesWithoutObservedPlayerDamage??0} none observed`,'Attackable-window k_EUserMsg_Damage observation only; none observed is not a miss or ignored opportunity.'); }
    case 'orb_shooter': { const d=model.flyingSouls?.summary?.damageObservation??{},row=(d.byPlayer??[]).find(x=>Number(x.controllerEntityIndex)===Number(p.identity?.controllerEntityIndex)); return v(row?.damagedEpisodes??0,`${row?.damageMessages??0} observed damage messages by selected player · no projectile-causality claim`); }
    case 'orb_multi_hit': { const d=model.flyingSouls?.summary?.damageObservation??{}; return v(d.multiMessageEpisodes??0,`${d.totalObservedPlayerDamageMessages??0} total observed player-attributed damage messages`); }
    ...
    case 'ground_soul_vacuum_target': { const target=model.groundSoulLifecycle?.summary?.physicalVacuumTargets,row=(target?.byPlayer??[]).find(x=>Number(x.controllerEntityIndex)===Number(p.identity?.controllerEntityIndex));return v(row?.resolvedPhysicalTargets??0,`${target?.resolvedPhysicalTargets??0}/${target?.targetedActivations??0} replay-wide target links resolved · physical target only`); }
...
async function latestNextPrimaryReady(replay,playerName,offset){
  try{
    ...
    for(let i=rows.length-1;i>=0;i--){
      const r=rows[i];
      const next=finite(r?.nextPrimaryAttack ?? r?.observedWeaponState?.nextPrimaryAttack ?? r?.directRuntime?.after?.nextPrimaryAttack ?? r?.directRuntime?.after?.m_flNextPrimaryAttack);
      if(next!==null){
...
function finite(x){if(x===null||x===undefined||x==='')return null;const n=Number(x);return Number.isFinite(n)?n:null;}
...
export function getMetricAvailability(model,p,id){return p?.metricAvailability?.[id]??model?.metricAvailability?.[id]??{available:false,reason:'Production data has not been validated.'};}
export function metricValue(id,ctx){
 ...
 if(stateFields[id]&&atState?.[stateFields[id]]==null)return{wired:true,available:false,value:'Unavailable',detail:'Field not observed at or before selected time.',availability};
 if(id==='next_primary_ready'&&!weaponReady)return{wired:true,available:true,value:'N/A',detail:'No discharge observed at or before selected time.',scope:'at_selected_time',availability};
 const result=rawMetricValue(id,{...ctx,weaponReady});
 return{...result,available:true,scope:selected.has(id)?'at_selected_time':'full_observed_match',availability};
}
export { clock, escapeHtml, apiJson };
