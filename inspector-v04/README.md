...
The current contract contains 124 A metrics: 102 core and 22 extended. The scoped promotions are `ground_soul_vacuum_target` (resolved physical attraction targets only), `reload_state` (direct observed reload state/transitions), `active_fire_mode` (raw numeric mode value), `burst_continuous_state` (raw continuous-shot and burst-remaining counters), `trooper_base_types` (legacy ID narrowed to direct raw subclass-ID counts), `trooper_team_lane` (direct raw numeric team/lane-pair counts), `trooper_orbs` (mutually unique source-linked CItemXP episodes), `orb_attackable_window` (direct attackable-time difference), `orb_shot_no_shot` (observed-damage/no-observed-damage episode distribution), `orb_shooter` (selected-player damaged-episode count under its legacy ID), `orb_multi_hit` (multiple-message episodes), `orb_multi_player` (multiple-player episodes), `orb_mixed_team` (mixed-team episodes), `orb_hits` (selected-player damage-message count), `orb_secure` (single-team opposite-orb-team first-hit credit), and `orb_deny` (single-team same-orb-team first-hit credit). Flying-soul damage observations require exact victim entity, a tick inside the episode-relative direct attackable window, and a unique sampled-player identity. Mixed-team winners, no-damage outcomes, named Trooper types, lane names, variants, rewards, automatic awards, projectile causality, misses, accuracy, visibility, and opportunity remain research/B.
