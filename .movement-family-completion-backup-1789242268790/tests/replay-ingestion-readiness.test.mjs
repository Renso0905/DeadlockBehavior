...
test('one-step ingestion readiness is driven by canonical A count',()=>assert.equal(A_TOTAL,89));
...
test('new DEM with no manifest is UNPROCESSED and requires ingestion',async()=>{
  ...
  assert.equal(r.state,'UNPROCESSED');assert.equal(r.authoritativeTotal,89);assert.equal(r.completeAuthoritative,0);
...
test('89/89 current manifest is READY',async()=>{
  const {root,output}=await fixtureWithManifest({manifestTotal:89,complete:89,runStatus:'COMPLETE'});
  const r=await replayReadiness({repoRoot:root,outputRoot:output,replayName:'match1',authoritativeTotal:A_TOTAL});
  assert.equal(r.state,'READY');assert.equal(r.completeAuthoritative,89);assert.equal(r.missingAuthoritative,0);assert.equal(r.contractCurrent,true);
});

test('current contract with failed coverage is INCOMPLETE',async()=>{
  const {root,output}=await fixtureWithManifest({manifestTotal:89,complete:88,failed:1,runStatus:'FAILED_REQUIRED_STEP'});
  const r=await replayReadiness({repoRoot:root,outputRoot:output,replayName:'match1',authoritativeTotal:A_TOTAL});
  assert.equal(r.state,'INCOMPLETE');assert.equal(r.completeAuthoritative,88);assert.equal(r.failedAuthoritative,1);
});
...
test('old complete manifest is STALE_CONTRACT rather than silently READY',async()=>{
  ...
  const r=await replayReadiness({repoRoot:root,outputRoot:output,replayName:'match1',authoritativeTotal:A_TOTAL});
  assert.equal(r.state,'STALE_CONTRACT');assert.equal(r.manifestAuthoritativeTotal,80);assert.equal(r.authoritativeTotal,89);assert.equal(r.contractCurrent,false);
});
