...
test('production capability contract owns every A metric exactly once',()=>{
  ...
  assert.equal(new Set(production).size,100);
  ...
  assert.equal(coreA.length,91);assert.equal(extendedA.length,9);assert.equal(supported.flatMap(c=>c.metricIds).length,100);
});

test('pipeline requires fresh outputs and records 91/91 Core A plus 9/9 Extended A coverage',async()=>{
  ...
  assert.equal(result.productionManifest.coverage.completeAuthoritative,100);assert.equal(result.productionManifest.coverage.authoritativeTotal,100);assert.equal(result.productionManifest.coverage.notSupportedAuthoritative,0);assert.equal(result.productionManifest.coverage.failedAuthoritative,0);assert.equal(result.productionManifest.coverage.blockedAuthoritative,0);assert.equal(result.productionManifest.coverage.unclassifiedAuthoritative,0);
  assert.equal(result.productionManifest.coverage.core.completeAuthoritative,91);assert.equal(result.productionManifest.coverage.core.authoritativeTotal,91);assert.equal(result.productionManifest.coverage.extended.completeAuthoritative,9);assert.equal(result.productionManifest.coverage.extended.authoritativeTotal,9);
  const disk=JSON.parse(await readFile(join(root,'output','fixture','production_manifest_v01.json'),'utf8'));assert.equal(disk.coverage.authoritativeTotal,100);assert.equal(disk.coverage.core.authoritativeTotal,91);assert.equal(disk.coverage.extended.authoritativeTotal,9);
});
