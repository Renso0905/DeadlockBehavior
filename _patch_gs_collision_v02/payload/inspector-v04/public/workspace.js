...
async function renderWorkspace(){
  ...
  try{
    ...
    const [audit,collisionAudit]=await Promise.all([needsAudit?getWorkspaceAudit(replay):Promise.resolve(null),needsCollisionAudit?getWorkspaceCollisionAudit(replay):Promise.resolve(null)]);
    ...
    main.innerHTML=`
      ...
      <section class="ws-grid" data-cols="${escapeHtml(cols)}">
        ${workspaceState.panels.map((panel,index)=>renderPanel(panel,index,model,registry,currentTime,workspaceState.baseline,audit,collisionAudit)).join('')}
      </section>
...
function renderPanel(panel,index,model,registry,currentTime,baselineTime,audit,collisionAudit){
...
function ensureWorkspacePanels(model){
  ...
  for(const p of workspaceState.panels){
    p.kind=p.kind==='spatial'?'spatial':p.kind==='groundSoulAudit'?'groundSoulAudit':p.kind==='groundSoulCollision'?'groundSoulCollision':'metric';
    ...
    if(!['groundSoulAudit','groundSoulCollision'].includes(p.kind)&&!players.some(x=>String(x.playerName)===String(p.playerName)))p.playerName=selected;
  }
}
