...
test('production capability contract owns every A metric exactly once',()=>{
  ...
  assert.equal(PRODUCTION_CAPABILITIES.filter(c=>c.productionStatus==='supported').flatMap(c=>c.metricIds).length,49);
});

test('pipeline requires fresh outputs and records 49/68 production coverage',async()=>{
  ...
  assert.equal(result.productionManifest.coverage.completeAuthoritative,49);
  assert.equal(result.productionManifest.coverage.notSupportedAuthoritative,19);
