...
export function normalizeWeaponEvent(row, index = 0) {
  ...
  return {
    ...
    clip: finite(firstDefined(
      row?.observedWeaponState?.clip,
      fieldValue(row, 'm_iClip'),
      deepFindExactKey(row, 'clip'),
    )),
    bonusClip: finite(firstDefined(
      row?.observedWeaponState?.bonusClip,
      fieldValue(row, 'm_iBonusClip'),
      deepFindExactKey(row, 'bonusClip'),
    )),
    ammoFrac: finite(firstDefined(
      row?.observedWeaponState?.ammoFraction,
      fieldValue(row, 'm_flAmmoFrac'),
      deepFindExactKey(row, 'ammoFraction'),
    )),
    inReload: booleanOrNull(firstDefined(
      row?.observedWeaponState?.inReload,
      fieldValue(row, 'm_bInReload'),
