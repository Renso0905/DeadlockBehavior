...
test('production capability contract owns every A metric exactly once',()=>{
  ...
  assert.equal(new Set(production).size,76);
  ...
  assert.equal(coreA.length,68);assert.equal(extendedA.length,8);assert.equal(supported.flatMap(c=>c.metricIds).length,76);
});

test('pipeline requires fresh outputs and records 68/68 Core A plus 8/8 Extended A coverage',async()=>{
  ...
  assert.equal(result.productionManifest.coverage.completeAuthoritative,76);assert.equal(result.productionManifest.coverage.authoritativeTotal,76);assert.equal(result.productionManifest.coverage.notSupportedAuthoritative,0);assert.equal(result.productionManifest.coverage.failedAuthoritative,0);assert.equal(result.productionManifest.coverage.blockedAuthoritative,0);assert.equal(result.productionManifest.coverage.unclassifiedAuthoritative,0);
  assert.equal(result.productionManifest.coverage.core.completeAuthoritative,68);assert.equal(result.productionManifest.coverage.core.authoritativeTotal,68);assert.equal(result.productionManifest.coverage.extended.completeAuthoritative,8);assert.equal(result.productionManifest.coverage.extended.authoritativeTotal,8);
  const disk=JSON.parse(await readFile(join(root,'output','fixture','production_manifest_v01.json'),'utf8'));assert.equal(disk.coverage.authoritativeTotal,76);assert.equal(disk.coverage.core.authoritativeTotal,68);assert.equal(disk.coverage.extended.authoritativeTotal,8);
});
