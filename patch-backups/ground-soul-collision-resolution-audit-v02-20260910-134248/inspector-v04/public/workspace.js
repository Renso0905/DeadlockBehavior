...
document.querySelector('#replaySelect')?.addEventListener('change',()=>{workspaceModel=null;workspaceReplay=null;workspaceAudit=null;workspaceAuditReplay=null;queueWorkspaceRender(100);});
document.querySelector('#refreshBtn')?.addEventListener('click',()=>{workspaceModel=null;workspaceReplay=null;workspaceAudit=null;workspaceAuditReplay=null;queueWorkspaceRender(500);});
...
async function renderWorkspace(){
  ...
  try{
    ...
    const audit=needsAudit?await getWorkspaceAudit(replay):null;
    ...
    main.innerHTML=`
      ...
      <section class="ws-grid" data-cols="${escapeHtml(cols)}">
        ${workspaceState.panels.map((panel,index)=>renderPanel(panel,index,model,registry,currentTime,workspaceState.baseline,audit)).join('')}
      </section>
...
function renderPanel(panel,index,model,registry,currentTime,baselineTime,audit){
...
function ensureWorkspacePanels(model){
  ...
  for(const p of workspaceState.panels){
    p.kind=p.kind==='spatial'?'spatial':p.kind==='groundSoulAudit'?'groundSoulAudit':'metric';
    ...
    if(p.kind!=='groundSoulAudit'&&!players.some(x=>String(x.playerName)===String(p.playerName)))p.playerName=selected;
  }
}
