...
test('production capability contract owns every A metric exactly once',()=>{
  ...
  assert.equal(PRODUCTION_CAPABILITIES.find(c=>c.id==='health_regen').productionStatus,'supported');
  ...
  assert.equal(PRODUCTION_CAPABILITIES.filter(c=>c.productionStatus==='supported').flatMap(c=>c.metricIds).length,70);
...
test('pipeline requires fresh outputs and records 68/68 Core A plus 2/2 Extended A coverage',async()=>{
  ...
  await writeFile(healthProducer,`import {mkdir,writeFile} from 'node:fs/promises';import {join} from 'node:path';const root=process.argv[2],r=process.argv[3];await mkdir(join(root,'output',r),{recursive:true});await writeFile(join(root,'output',r,'runtime_health_regen_production_v01.json'),'{}\\n');await writeFile(join(root,'output',r,'runtime_health_regen_events_v01.jsonl'),'');`);  const trooperProducer=join(root,'trooper-producer.mjs');
  await writeFile(trooperProducer,`import {mkdir,writeFile} from 'node:fs/promises';import {join} from 'node:path';const root=process.argv[2],r=process.argv[3];await mkdir(join(root,'output',r),{recursive:true});await writeFile(join(root,'output',r,'runtime_trooper_deaths_production_v01.json'),'{}\n');await writeFile(join(root,'output',r,'runtime_trooper_death_events_v01.jsonl'),'');`);  const itemProducer=join(root,'item-producer.mjs');
  ...
  const pipeline={version:'TEST',manifestFile:'production_manifest_v01.json',steps:[
    ...
    {id:'health',label:'health',capability:'health_regen',required:true,dependsOn:['core'],args:[healthProducer,'{repoRoot}','{replay}'],expectedOutputs:[{path:'output/{replay}/runtime_health_regen_production_v01.json',minBytes:2},{path:'output/{replay}/runtime_health_regen_events_v01.jsonl',minBytes:0}]},
    {id:'trooper',label:'trooper',capability:'runtime_trooper_death_events',required:true,dependsOn:['core'],args:[trooperProducer,'{repoRoot}','{replay}'],expectedOutputs:[{path:'output/{replay}/runtime_trooper_deaths_production_v01.json',minBytes:2},{path:'output/{replay}/runtime_trooper_death_events_v01.jsonl',minBytes:0}]},
  ...
  assert.equal(result.results.find(r=>r.id==='health').status,'complete');
  assert.equal(result.results.find(r=>r.id==='trooper').status,'complete');
