...
test('production capability contract owns every A metric exactly once',()=>{
  ...
  assert.equal(PRODUCTION_CAPABILITIES.filter(c=>c.productionStatus==='supported').flatMap(c=>c.metricIds).length,68);
});

test('pipeline requires fresh outputs and records 68/68 production coverage',async()=>{
  ...
  assert.equal(result.productionManifest.coverage.completeAuthoritative,68);
