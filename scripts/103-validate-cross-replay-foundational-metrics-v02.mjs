...
const OUTPUT_MARKDOWN_PATH =
  resolve(
    'output',
    'cross_replay',
    'foundational_replication_metrics_v02.md'
  );
...
//     distance3D(...);
//
// inside buildStrictDeathActivationMatches().
//
// The local const shadows the helper function of the same name.
// JavaScript therefore attempts to read the uninitialized local
// binding while it is still in its temporal dead zone.
//
// V02 renames the local value:
//
//   edgeDistance3D
//
// No telemetry or extraction outputs are affected.
// No replay reparse is required.
// ============================================================
...
console.log(
  'CROSS-REPLAY FOUNDATIONAL REPLICATION METRICS V0.2'
);
...
console.log(
  'Fixed distance3D local-variable shadowing in strict death↔activation matching.'
);


console.log(
  'No replay reparse or event re-extraction required.'
);
...
const summary = {
  ...
  v01Correction: {
    ...
    issue:
      'A local const named distance3D shadowed the distance3D helper function in buildStrictDeathActivationMatches().',

    error:
      'ReferenceError: Cannot access distance3D before initialization',

    correction:
      'The local scalar is now named edgeDistance3D.',

    extractionDataAffected:
      false,
...
async function analyzeReplay(
  ...
  const summary = {
    ...
    version:
      'FOUNDATIONAL_REPLICATION_METRICS_V02',
  ...
  for (
    ...
    for (
      ...
      for (
        ...
        // V02 FIX
        //
        // Do NOT name this scalar "distance3D".
        //
        // That would shadow the helper function distance3D()
        // throughout this block and trigger the temporal-dead-zone
        // ReferenceError seen in V01.
        // ------------------------------------------------------

        const edgeDistance3D =
          distance3D(
            death.position,
            activation.position
          );
