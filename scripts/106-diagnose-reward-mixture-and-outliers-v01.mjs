...
const OUTPUT_JSON_PATH =
  resolve(
    'output',
    'cross_replay',
    'reward_mixture_outlier_diagnostic_v01.json'
  );
...
const OUTPUT_MARKDOWN_PATH =
  resolve(
    'output',
    'cross_replay',
    'reward_mixture_outlier_diagnostic_v01.md'
  );
...
//
// is withdrawn pending mixture/outlier analysis.
//
// ============================================================
...
console.log(
  'REWARD MIXTURE / OUTLIER DIAGNOSTIC V0.1'
);
...
console.log(
  'Previous patch-era interpretation is WITHDRAWN pending this audit.'
);
...
for (
  ...
  console.log(
    `  case winner POST/PRE:     ${row.singleModelCaseWinners.post}/${row.singleModelCaseWinners.pre}`
  );


  console.log(
    `  case winner POST rate:    ${formatPercent(
      row.singleModelCaseWinners.postRate
    )}`
  );
  ...
  console.log(
    `  POST mixture combined:    ${postMix.combinedAssignments}/${row.eligibleCases} (${formatPercent(
...
function analyzeCaseSet(
  ...
  //
  // Compare best robust rounding for each era.
  // ----------------------------------------------------------
...
function predictCombined(
  ...
  // ----------------------------------------------------------
  // Treat the two candidate rewards as separately integerized
  // reward components.
  //
  // This is more mechanically meaningful than rounding their
  // continuous sum once.
  // ----------------------------------------------------------
...
// TRIMMED RMSE
//
// Removes the largest absolute residual fraction.
//
// Used only as a robustness diagnostic.
// ============================================================
...
function buildMarkdown(
  ...
  lines.push(
    '# Reward Mixture / Outlier Diagnostic'
  );
  ...
  lines.push(
    'The Script105 pre-June-30 conclusion is withdrawn because it selected the patch era using RMSE alone despite contradictory MAE and near-exact accuracy.'
  );
