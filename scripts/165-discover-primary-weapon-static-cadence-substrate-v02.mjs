...
const VERSION = 'PRIMARY_WEAPON_STATIC_CADENCE_SUBSTRATE_V02';
...
const OUTPUT_PATH = resolve('output', 'cross_replay', 'primary_weapon_static_cadence_substrate_v02.json');
...
const broadV01Bindings = findHeroPrimaryWeaponBindings(script131);
const heroBindings = findSelectableHeroPrimaryWeaponBindings(script131);
const unresolvedSelectableBindings = findHeroPrimaryWeaponBindings({
  unresolvedSelectableState: script131?.unresolvedSelectableState ?? []
});
const strictHeroIds = new Set(heroBindings.map(row => row.heroId));
const v01ExcludedBindings = broadV01Bindings.filter(row => !strictHeroIds.has(row.heroId));
const unresolvedSelectableIds = new Set(unresolvedSelectableBindings.map(row => row.heroId));
const excludedExactlyMatchesUnresolved =
  v01ExcludedBindings.length === unresolvedSelectableBindings.length
  && v01ExcludedBindings.every(row => unresolvedSelectableIds.has(row.heroId));
...
console.log('');
console.log('========================================================');
console.log('PRIMARY WEAPON STATIC CADENCE SUBSTRATE V0.2');
console.log('========================================================');
console.log('');
console.log(`Hero authority:                  Script131 V01 (${script131?.counts?.selectableHeroes ?? 'n/a'} selectable heroes)`);
console.log(`V01 broad bindings:              ${broadV01Bindings.length}`);
console.log(`V02 strict selectable bindings:  ${heroBindings.length}`);
console.log(`V01 excluded bindings:           ${v01ExcludedBindings.length}`);
...
const temporaryDirectory = mkdtempSync(join(tmpdir(), 'deadlock-primary-weapon-static-v02-'));
...
try {
  ...
  const checks = {
    ...
    script131SelectableHeroCount: check(script131?.counts?.selectableHeroes ?? null, 42, script131?.counts?.selectableHeroes === 42),
    script131HeroesArrayMatchesSelectableCount: check(
      Array.isArray(script131?.heroes) ? script131.heroes.length : null,
      script131?.counts?.selectableHeroes ?? null,
      Array.isArray(script131?.heroes) && script131.heroes.length === script131?.counts?.selectableHeroes
    ),
    selectableHeroPrimaryBindings: check(heroBindings.length, 42, heroBindings.length === 42),
    v01BroadBindingOverreachDiagnosed: check(
      { broad: broadV01Bindings.length, strict: heroBindings.length, excluded: v01ExcludedBindings.length },
      { broad: 43, strict: 42, excluded: 1 },
      broadV01Bindings.length === 43 && heroBindings.length === 42 && v01ExcludedBindings.length === 1
    ),
    v01ExcludedBindingMatchesScript131UnresolvedSelectable: check(
      excludedExactlyMatchesUnresolved,
      true,
      excludedExactlyMatchesUnresolved
    ),
  ...
  const status = integrityPass
    ...
    : 'PRIMARY_WEAPON_STATIC_CADENCE_SUBSTRATE_V02_REQUIRES_DIAGNOSIS';
  ...
  const summary = {
    ...
    scope: {
      purpose: 'Correct Script165 V01 cohort overreach and resolve build-bound primary-weapon static cadence/operation fields for the Script131 player-selectable hero cohort.',
      v01Correction: 'V01 recursively included one Script131 unresolved-selectable hero. V02 consumes only Script131.heroes entries with playerSelectable === true; V01 is preserved unchanged as diagnostic history.',
    ...
    counts: {
      ...
      broadV01Bindings: broadV01Bindings.length,
      v01ExcludedBindings: v01ExcludedBindings.length,
      unresolvedSelectableBindings: unresolvedSelectableBindings.length,
    ...
    v01CohortDiagnosis: {
      broadBindingCount: broadV01Bindings.length,
      strictSelectableBindingCount: heroBindings.length,
      excludedBindings: v01ExcludedBindings,
      unresolvedSelectableBindings,
      excludedExactlyMatchesUnresolved
    },
    ...
    nextStage: integrityPass
      ...
      : 'DIAGNOSE_STRICT_SELECTABLE_COHORT_OR_PRIMARY_WEAPON_RESOURCE_RESOLUTION'
  };
  ...
  console.log(`hero primary bindings (strict):   ${heroBindings.length}`);
  console.log(`V01 broad bindings:               ${broadV01Bindings.length}`);
  console.log(`V01 excluded as unresolved:       ${v01ExcludedBindings.length}`);
  for (const row of v01ExcludedBindings) {
    console.log(`  excluded heroId=${row.heroId} name=${row.displayName ?? 'n/a'} weapon=${row.primaryWeaponRecordKey}`);
  }
