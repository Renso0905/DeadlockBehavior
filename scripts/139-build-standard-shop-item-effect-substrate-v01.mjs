...
const VERSION = 'STANDARD_SHOP_ITEM_EFFECT_SUBSTRATE_V01';
...
//
// Script139 builds the effect substrate for ONLY that frozen catalog.
//
// It separates:
//
...
const OUTPUT_JSON_PATH = resolve('output', 'cross_replay', 'standard_shop_item_effect_substrate_v01.json');
...
const WEAPON_TOKEN_REGEX = /(FIRE_RATE|AMMO|CLIP|RELOAD|BULLET|PROJECTILE|WEAPON|SHOT|RANGE|SPREAD|VELOCITY)/i;
const WEAPON_FIELD_REGEX = /(fire.?rate|ammo|clip|reload|bullet|projectile|weapon|shot|range|spread|velocity)/i;
...
console.log('');
console.log('========================================================');
console.log('STANDARD SHOP ITEM EFFECT SUBSTRATE V0.1');
console.log('========================================================');
console.log('');
...
try {
    ...
    for (const catalogRow of catalogRows) {
        ...
        const weaponRelevantDirectStats = directProvidedStats.filter(row =>
            WEAPON_TOKEN_REGEX.test(String(row.providedPropertyType ?? ''))
            || WEAPON_FIELD_REGEX.test(String(row.propertyKey ?? ''))
        );

        const weaponRelevantNonDirectTokens = nonDirectModifierTokens.filter(token => WEAPON_TOKEN_REGEX.test(token));
        const weaponRelevantFields = assignmentFieldNames.filter(fieldName => WEAPON_FIELD_REGEX.test(fieldName));
        ...
        itemEffects.push({
            ...
            interpretation: {
                ...
                weaponStateRelevant: 'CANDIDATE_SUBSTRATE_ONLY_NOT_RUNTIME_EFFECTIVE_WEAPON_STATE'
            }
        });
    }
    ...
    const itemsWithWeaponStateEvidence = itemEffects.filter(row => row.weaponStateRelevant.hasAnyEvidence);
    ...
    const status = validationPass
        ? 'STANDARD_SHOP_ITEM_EFFECT_SUBSTRATE_V01_READY'
        : 'STANDARD_SHOP_ITEM_EFFECT_SUBSTRATE_V01_REQUIRES_DIAGNOSIS';

    const nextStage = validationPass
        ? 'RESOLVE_DIRECT_STAT_SEMANTICS_AND_CLASSIFY_CONDITIONAL_EFFECT_FAMILIES_THEN_DISCOVER_REPLAY_ITEM_OWNERSHIP'
    ...
    const output = {
        ...
        interpretation: {
            ...
            weaponState:
                'Weapon-relevant tokens and fields identify items that may alter ammo, fire rate, reload, projectile/bullet behavior, range, spread, shot timing or weapon damage. Runtime applicability and exact formulas remain unresolved.',
            ownership:
    ...
    console.log(`weapon-state relevant:      ${itemsWithWeaponStateEvidence.length}`);
