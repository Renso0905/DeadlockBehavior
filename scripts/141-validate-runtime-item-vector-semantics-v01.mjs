...
const VERSION = 'RUNTIME_ITEM_VECTOR_SEMANTIC_VALIDATION_V01';
...
//
// Script141 asks the next semantic question:
//
...
const OUTPUT_PATH = resolve('output', replayName, 'runtime_item_vector_semantic_validation_v01.json');
...
console.log('');
console.log('========================================================');
console.log('RUNTIME ITEM VECTOR SEMANTIC VALIDATION V0.1');
console.log('========================================================');
console.log('');
...
// These gates intentionally require temporal separation from same-player
// placebo, not merely that both series are active. Amount agreement and
// shop-zone association remain supporting evidence because upgrades,
// remote/shop mechanics, and category currency representation can complicate
// exact one-to-one price deltas.
const checks = {
  ...
  shopZoneStateMostlyKnown: check(shopZoneKnownRate, '>=0.80', shopZoneKnownRate !== null && shopZoneKnownRate >= 0.80),
};

const validationPass = Object.values(checks).every(row => row.pass);
const status = validationPass
  ? 'RUNTIME_ITEM_VECTOR_V01_STRONG_SINGLE_REPLAY_OWNERSHIP_SEMANTICS'
  : 'RUNTIME_ITEM_VECTOR_V01_SEMANTIC_VALIDATION_REQUIRES_DIAGNOSIS';
...
const output = {
  ...
  shopZoneEvidence: {
    ...
    interpretation: 'Shop-zone state is supporting context, not a hard purchase definition. A false state does not by itself disprove item ownership because shop mechanics and state timing require independent validation.',
  },
  ...
  interpretation: {
    supported: validationPass
      ? 'In this replay, m_vecUpgrades has strong single-replay semantic support as the player runtime item-ownership vector because catalog-resolved additions are tightly associated with same-player spend telemetry beyond shifted-time placebo.'
      : 'The artifact independently measures whether m_vecUpgrades transitions align with same-player transaction/shop telemetry. Failed gates must be diagnosed before promoting runtime item ownership.',
  ...
  nextStage: validationPass
    ...
    : 'DIAGNOSE_SPEND_BUCKET_SHOP_ZONE_OR_PAWN_MAPPING_BEFORE_OWNERSHIP_PROMOTION',
};
