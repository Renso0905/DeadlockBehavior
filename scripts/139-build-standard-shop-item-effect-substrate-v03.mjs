...
const VERSION = 'STANDARD_SHOP_ITEM_EFFECT_SUBSTRATE_V03';
...
//
// Script139 V03 corrects V02 zero-valued provided-property saturation while retaining the location-aware non-direct token separation and effect substrate for ONLY that frozen catalog.
//
// It separates:
//
...
const OUTPUT_JSON_PATH = resolve('output', 'cross_replay', 'standard_shop_item_effect_substrate_v03.json');
...
console.log('');
console.log('========================================================');
console.log('STANDARD SHOP ITEM EFFECT SUBSTRATE V0.3');
console.log('========================================================');
console.log('');
...
try {
    ...
    for (const catalogRow of catalogRows) {
        ...
        const meaningfulDirectProvidedStats = directProvidedStats.filter(isMeaningfulProvidedStat);
        const zeroOrEmptyDirectProvidedStats = directProvidedStats.filter(row => !isMeaningfulProvidedStat(row));

        const weaponRelevantDirectStats = meaningfulDirectProvidedStats.filter(row =>
        ...
        itemEffects.push({
            ...
            zeroOrEmptyDirectProvidedStats,
            directProvidedStatCount: directProvidedStats.length,
            meaningfulDirectProvidedStatCount: meaningfulDirectProvidedStats.length,
            ...
            interpretation: {
                directProvidedStats: 'RAW_DIRECT_PROVIDED_PROPERTY_SUBSTRATE_INCLUDING_ZERO_VALUED_SCHEMA_DEFAULTS',
                meaningfulDirectProvidedStats: 'NONZERO_OR_SYMBOLIC_DIRECT_EFFECT_SUBSTRATE_USED_FOR_MECHANIC_RELEVANCE',
    ...
    const itemsWithMeaningfulDirectProvidedStats = itemEffects.filter(row => row.meaningfulDirectProvidedStatCount > 0);
    ...
    const meaningfulDirectProvidedTokenUniverse = summarizeTokenUniverse(
        itemEffects,
        row => row.meaningfulDirectProvidedStats.map(stat => stat.providedPropertyType).filter(Boolean)
    );
    ...
    const validationChecks = {
        ...
        meaningfulDirectProvidedStatsFound: check(
            itemsWithMeaningfulDirectProvidedStats.length,
            '>0',
            itemsWithMeaningfulDirectProvidedStats.length > 0
        ),
    ...
    const status = validationPass
        ...
        : 'STANDARD_SHOP_ITEM_EFFECT_SUBSTRATE_V03_REQUIRES_DIAGNOSIS';
    ...
    const output = {
        ...
        counts: {
            ...
            itemsWithMeaningfulDirectProvidedStats: itemsWithMeaningfulDirectProvidedStats.length,
            ...
            meaningfulDirectProvidedTokenTypes: meaningfulDirectProvidedTokenUniverse.length,
        ...
        universes: {
            ...
            meaningfulDirectProvidedPropertyTypes: meaningfulDirectProvidedTokenUniverse,
        ...
        interpretation: {
            directProvidedProperties:
                'm_mapAbilityProperties rows with m_eProvidedPropertyType are retained as the raw direct stat schema. Numeric zero/empty provided-property rows are treated as schema/default rows for mechanic-relevance classification, while nonzero numeric or symbolic/formula values remain effect evidence. Exact composition/stacking semantics remain to be validated before use as effective PlayerState(t).',
            nonDirectModifierTokens:
