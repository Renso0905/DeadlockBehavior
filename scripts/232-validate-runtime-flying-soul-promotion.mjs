...
for(const replayName of names){
  ...
  check(manifest?.runStatus==='COMPLETE','published manifest is not COMPLETE');check(manifest?.coverage?.completeAuthoritative===124,`manifest A coverage is ${manifest?.coverage?.completeAuthoritative??'missing'}, expected 124`);
